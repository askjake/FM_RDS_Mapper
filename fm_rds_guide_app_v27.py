...
class NCO:
    ...
    def mix_down(self, x: np.ndarray) -> np.ndarray:
        if x.size == 0 or self.freq == 0.0:
            return x.astype(np.complex64, copy=False)
        n = np.arange(x.size, dtype=np.float32)
        ph = self.phase + (2.0 * np.pi * self.freq / self.fs) * n
        osc = np.exp(-1j * ph).astype(np.complex64)
        # Advance phase accumulator
        self.phase = float((ph[-1] + (2.0 * np.pi * self.freq / self.fs)) % (2.0 * np.pi))
...
def decode_rds_for_station(cfg: CaptureConfig, event_q: "queue.Queue[dict]", db_path: str, power_db: float | None) -> None:
    ...
    try:
        ...
        emit_event(event_q, {"type": "error", "msg": f"Capture loop exception: {e}", "ts": iso_utc_now()})
    finally:
        transfer.stop()
